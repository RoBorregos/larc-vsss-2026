...
//
//     const std::vector<cv::Point>& global_points = blob_calibrator->get_points();
...
//         preprocessing::apply_preprogrammed_filters(robot_roi, app_data->color_params);
//         cv::cvtColor(robot_roi, robot_roi, cv::COLOR_HSV2BGR);
//
//         cv::Mat zoomed_view;
...
//            local_points.reserve(global_points.size());
//
//            for (const auto& gp : global_points) {
//               if (!rect.contains(gp)) continue;
//
...
//                   cv::circle(zoomed_view, lp, 3, cv::Scalar(0, 0, 255), -1);
//                }
//            }
//         }
//
...
//        }
//     }
//     window_names = current_window_names;
}
//
